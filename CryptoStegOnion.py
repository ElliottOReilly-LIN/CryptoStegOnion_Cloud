...
#  most common type of hashing used for file integrity checks are MD5, SHA-2 and CRC32.
        ...
        withSHA128 = ttk.Checkbutton(child, text='SHA-256').grid(row=2, column=1, padx=25, pady=5, sticky='e')
        sha128 = StringVar()
        sha128.set("SHA 128"),
        ...
        def getvalue():
            secretMSG = mystring.get()  # Save the message a text fileterry

            runOpenSSL(secretMSG)  # Pass the message to OpenSSL
...
class openSSLWindow:
    ...
    def window3(self):
        ...
        # from googleCloudSend import sendImage
        # This is where we are exporting to the cloud bucket
         #   sendImage()
        ...
        ttk.Button(child, text="Click to Send message to stegooPI.!!", style='Header.TLabel', command=callStegapy).grid(
